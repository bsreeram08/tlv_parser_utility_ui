...
export function EnhancedTestsDrawer({
  ...
  const [filteredTests, setFilteredTests] = useState<
    Array<SavedTlvTest | SavedIsoTest>
  >([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [selectedTest, setSelectedTest] = useState<
    SavedTlvTest | SavedIsoTest | null
  >(null);
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [showFavoritesOnly, setShowFavoritesOnly] = useState(false);
  const [sortOption, setSortOption] = useState<
    "newest" | "oldest" | "name" | "lastAccessed"
  >("newest");
  ...
  const toggleFavorite = async (
    test: SavedTlvTest | SavedIsoTest
  ): Promise<void> => {
    if (!test.id) return;
        ...
        <DrawerContent>
          <div className="mx-auto max-w-5xl w-full">
            <DrawerHeader>
              ...
              <DrawerDescription>
                View and load your previously saved{" "}
                {testType === "tlv" ? "TLV" : "ISO 8583"} tests
              </DrawerDescription>
                    ...
                    <SelectContent>
                      ...
                      {availableCategories
                        ...
                        .map((category) => (
                          <SelectItem
                            key={category}
                            value={category || "category-" + Math.random()}
                          >
                            {category}
                          </SelectItem>
                        ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-4">
                  {filteredTests.map((test, index) => (
                    <Card
                      key={test.id || index}
                      className={`group ${
                        test.favorite ? "border-yellow-300" : ""
                      }`}
                    >
